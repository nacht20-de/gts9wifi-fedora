...
static void glink_spss_tx_kick(struct qcom_glink_pipe *glink_pipe)
{
	...
	mbox_send_message(pipe->spss->mbox_chan, NULL);
	mbox_client_txdone(pipe->spss->mbox_chan, 0);
}
...
{
	struct qcom_glink_spss *spss =
		container_of(dev, struct qcom_glink_spss, dev);
						 ...
						 struct device_node *node)
{
	...
	rx_pipe->native.peek = glink_spss_rx_peak;
	...
	*tx_pipe->head = 0;
	...
	ret = devm_request_irq(dev, spss->irq, qcom_glink_spss_intr,
