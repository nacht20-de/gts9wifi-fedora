...
#define GTS9U_SPSS_MEM_BASE	0x9b100000ULL
#define GTS9U_SPSS_MEM_SIZE	0x181000

/* err_status definitions                       */
#define PBL_LOG_VALUE                 (0xef000000)
#define PBL_LOG_MASK                  (0xff000000)

#define to_glink_subdev(d) container_of(d, struct qcom_spss_glink, subdev)
...
static int spss_load_state_toggle(struct qcom_spss *spss, bool enable)
{
	if (!spss->qmp)
		return 0;

	return qmp_send(spss->qmp,
			"{class: image, res: load_state, name: %s, val: %s}",
			spss->qmp_name, enable ? "on" : "off");
}
...
{
	struct qcom_spss_glink *glink = to_glink_subdev(subdev);
...
static void clear_wdog(struct qcom_spss *spss)
{
	...
	dev_err(spss->dev, "rproc recovery state: %s\n", spss->rproc->recovery_disabled ?
		"disabled; leaving the remote processor crashed" :
		"enabled; scheduling recovery");
...
static int spss_stop(struct rproc *rproc)
{
	...
	if (ret)
		dev_err(spss->dev, "remoteproc %s failed to shut down: %d\n",
			rproc->name, ret);

	mask_scsr_irqs(spss);
	if (spss->qmp)
		spss_load_state_toggle(spss, false);
...
static int spss_attach(struct rproc *rproc)
{
	...
	if (spss->qmp) {
		ret = spss_load_state_toggle(spss, true);
		if (ret) {
...
static int spss_start(struct rproc *rproc)
{
	...
	/* Signal AOP about spss status. */
	if (spss->qmp) {
		status = spss_load_state_toggle(spss, true);
		if (status) {
			dev_err(spss->dev,
	...
	if (ret) {
		dev_err(spss->dev, "auth and reset failed for remoteproc %s: %d\n",
			rproc->name, ret);
		goto disable_spss;
	}
	...
	read_sp2cl_debug_registers(spss);
	if (!ret)
		dev_err(spss->dev, "%s start timed out\n", rproc->name);
	ret = ret ? 0 : -ETIMEDOUT;

	/* if SPSS fails to start, signal AOP about spss status. */
disable_spss:
	if (ret && spss->qmp) {
		status = spss_load_state_toggle(spss, false);
		if (status)
			dev_err(spss->dev,
			"Failed to signal AOP about spss status [%d]\n", status);
	}
...
static int spss_alloc_memory_region(struct qcom_spss *spss)
{
	...
	if (!node) {
		spss->mem_phys = spss->mem_reloc = GTS9U_SPSS_MEM_BASE;
		spss->mem_size = GTS9U_SPSS_MEM_SIZE;
		spss->mem_region = devm_ioremap_wc(spss->dev, spss->mem_phys,
						    spss->mem_size);
		return spss->mem_region ? 0 : -EBUSY;
	}
...
static int qcom_spss_probe(struct platform_device *pdev)
{
	...
	/*
	 * SPSS has no SSCTL/shutdown-ack interrupt.  The downstream sysmon
	 * helper tolerates that, while mainline treats the unnamed generic IRQ
	 * as a malformed shutdown-ack and rejects the whole remoteproc.  SSR and
	 * GLINK are sufficient for this independently managed PAS image.
	 */
	spss->sysmon_subdev = NULL;
...
del_rproc:
	rproc_del(rproc);
remove_subdev:
	qcom_remove_glink_spss_subdev(rproc, &spss->glink_subdev);
...
static struct of_changeset gts9u_spss_ocs;
static struct device_node *gts9u_spss_node;
static bool gts9u_spss_ocs_initialized;
static bool gts9u_spss_ocs_applied;

static int gts9u_find_phandle(const char *compatible, phandle *handle,
			      struct device_node **held)
{
	struct device_node *node;
...
static int gts9u_publish_spss_node(void)
{
	static const char * const reg_names[] = {
		"sp2soc_irq_status", "sp2soc_irq_clr", "sp2soc_irq_mask",
		"rmb_err", "rmb_general_purpose", "rmb_err_spare2",
	};
	static const u32 regs[] = {
		0, 0x188101c, 0, 4, 0, 0x1881024, 0, 4,
		0, 0x1881028, 0, 4, 0, 0x188103c, 0, 4,
		0, 0x1881100, 0, 4, 0, 0x1882014, 0, 4,
	};
	static const char * const glink_reg_names[] = {
		"qcom,spss-addr", "qcom,spss-size",
	};
	static const u32 glink_regs[] = {
		0, 0x1885008, 0, 8, 0, 0x1885010, 0, 4,
	};
	struct device_node *soc = NULL, *rpmh = NULL, *aoss = NULL;
	struct device_node *ipcc = NULL, *gic = NULL, *glink, *existing;
	phandle rpmh_ph, aoss_ph, ipcc_ph, gic_ph;
	u32 ints[5], clocks[2], qmp[1], mboxes[3], glink_ints[4];
	int ret;

	existing = of_find_compatible_node(NULL, NULL, "qcom,kalama-spss-pas");
	if (existing) {
	...
	if (!soc)
		return -ENODEV;
	ret = gts9u_find_phandle("qcom,sm8550-rpmh-clk", &rpmh_ph, &rpmh);
	if (ret)
		goto out;
	ret = gts9u_find_phandle("qcom,sm8550-aoss-qmp", &aoss_ph, &aoss);
	if (ret)
		goto out;
	ret = gts9u_find_phandle("qcom,sm8550-ipcc", &ipcc_ph, &ipcc);
	if (ret)
		goto out;
	...
	of_changeset_init(&gts9u_spss_ocs);
	gts9u_spss_ocs_initialized = true;
	gts9u_spss_node = of_changeset_create_node(&gts9u_spss_ocs, soc,
						       "remoteproc-spss@1880000");
	if (IS_ERR(gts9u_spss_node)) {
		ret = PTR_ERR(gts9u_spss_node);
		gts9u_spss_node = NULL;
		goto destroy;
	}

	/* Mainline GICv3 uses the four-cell Qualcomm interrupt specifier. */
	ints[0] = gic_ph; ints[1] = 0; ints[2] = 0x160;
	ints[3] = 1; ints[4] = 0;
	clocks[0] = rpmh_ph; clocks[1] = 0;
	qmp[0] = aoss_ph;
	ret = of_changeset_add_prop_string(&gts9u_spss_ocs,
		gts9u_spss_node, "compatible", "qcom,kalama-spss-pas");
	if (!ret)
		ret = of_changeset_add_prop_u32_array(&gts9u_spss_ocs,
			gts9u_spss_node, "reg", regs, ARRAY_SIZE(regs));
	if (!ret)
		ret = of_changeset_add_prop_string_array(&gts9u_spss_ocs,
			gts9u_spss_node, "reg-names", reg_names,
			ARRAY_SIZE(reg_names));
	if (!ret)
		ret = of_changeset_add_prop_u32_array(&gts9u_spss_ocs,
			gts9u_spss_node, "interrupts-extended", ints,
			ARRAY_SIZE(ints));
	if (!ret)
		ret = of_changeset_add_prop_u32_array(&gts9u_spss_ocs,
			gts9u_spss_node, "clocks", clocks, ARRAY_SIZE(clocks));
	if (!ret)
		ret = of_changeset_add_prop_string(&gts9u_spss_ocs,
			gts9u_spss_node, "clock-names", "xo");
	if (!ret)
		ret = of_changeset_add_prop_u32_array(&gts9u_spss_ocs,
			gts9u_spss_node, "qcom,qmp", qmp, ARRAY_SIZE(qmp));
	if (!ret)
		ret = of_changeset_add_prop_bool(&gts9u_spss_ocs,
			gts9u_spss_node, "qcom,signal-aop");
	if (!ret) {
		u32 bits[] = { 0x18, 0x19 };
		ret = of_changeset_add_prop_u32_array(&gts9u_spss_ocs,
			gts9u_spss_node, "qcom,spss-scsr-bits", bits,
			ARRAY_SIZE(bits));
	}
	if (!ret)
		ret = of_changeset_add_prop_u32(&gts9u_spss_ocs,
			gts9u_spss_node, "qcom,extra-size", 0x1000);
	if (!ret)
		ret = of_changeset_add_prop_u32(&gts9u_spss_ocs,
			gts9u_spss_node, "#address-cells", 2);
	if (!ret)
		ret = of_changeset_add_prop_u32(&gts9u_spss_ocs,
			gts9u_spss_node, "#size-cells", 2);
	if (!ret)
		ret = of_changeset_add_prop_bool(&gts9u_spss_ocs,
			gts9u_spss_node, "ranges");
	if (ret)
		goto destroy;

	glink = of_changeset_create_node(&gts9u_spss_ocs,
					 gts9u_spss_node, "glink-edge");
	if (IS_ERR(glink)) {
		ret = PTR_ERR(glink);
		goto destroy;
	}
	mboxes[0] = ipcc_ph; mboxes[1] = 16; mboxes[2] = 0;
	glink_ints[0] = ipcc_ph; glink_ints[1] = 16;
	glink_ints[2] = 0; glink_ints[3] = 1;
	ret = of_changeset_add_prop_u32(&gts9u_spss_ocs, glink,
					"qcom,remote-pid", 8);
	if (!ret)
		ret = of_changeset_add_prop_u32_array(&gts9u_spss_ocs, glink,
			"mboxes", mboxes, ARRAY_SIZE(mboxes));
	if (!ret)
		ret = of_changeset_add_prop_u32_array(&gts9u_spss_ocs, glink,
			"interrupts-extended", glink_ints,
			ARRAY_SIZE(glink_ints));
	if (!ret)
		ret = of_changeset_add_prop_u32_array(&gts9u_spss_ocs, glink,
			"reg", glink_regs, ARRAY_SIZE(glink_regs));
	if (!ret)
		ret = of_changeset_add_prop_string_array(&gts9u_spss_ocs, glink,
			"reg-names", glink_reg_names, ARRAY_SIZE(glink_reg_names));
	if (!ret)
		ret = of_changeset_add_prop_string(&gts9u_spss_ocs, glink,
					    "label", "spss");
	if (ret)
		goto destroy;

	ret = of_changeset_apply(&gts9u_spss_ocs);
	if (ret)
		goto destroy;
	gts9u_spss_ocs_applied = true;
	/* The OF dynamic notifier creates the platform device synchronously. */
	ret = 0;
	goto out;

destroy:
	of_changeset_destroy(&gts9u_spss_ocs);
	gts9u_spss_ocs_initialized = false;
out:
	of_node_put(gic);
	of_node_put(ipcc);
	of_node_put(aoss);
...
static int __init qcom_spss_init(void)
{
	int ret;
	...
	if (ret)
		return ret;
	ret = gts9u_publish_spss_node();
	if (ret)
		platform_driver_unregister(&spss_driver);
	return ret;
}

static void __exit qcom_spss_exit(void)
{
	if (gts9u_spss_ocs_applied)
		of_changeset_revert(&gts9u_spss_ocs);
	if (gts9u_spss_ocs_initialized)
		of_changeset_destroy(&gts9u_spss_ocs);
	platform_driver_unregister(&spss_driver);
}

module_init(qcom_spss_init);
module_exit(qcom_spss_exit);
