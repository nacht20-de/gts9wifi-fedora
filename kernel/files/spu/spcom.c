...
/*
 * QTEE's TEE_IOC_SHM_ALLOC descriptors are anon-inode files rather than
 * DMA-BUFs. SPCOM only has to retain the SPU bootstrap buffer while it is
 * registered with sp_keymaster, so keep a file reference for that case. The
 * buffer remains owned, mapped and physically described by the TEE core.
 */
static struct file *spcom_get_tee_shm_file(int fd)
{
	...
	if (!file->f_path.dentry ||
	    strcmp(file->f_path.dentry->d_name.name, "tee_shm")) {
		fput(file);
...
static struct rproc *spcom_get_rproc(void)
{
	if (spcom_dev->rproc_prop)
		return rproc_get_by_phandle(
			be32_to_cpup(spcom_dev->rproc_prop->value));

	return rproc_get_by_child(&spcom_dev->pdev->dev);
}
...
static int spcom_handle_restart_sp_command(void *cmd_buf, int cmd_size)
{
	...
	spcom_dev->spss_rproc = spcom_get_rproc();
...
static int spcom_dmabuf_unlock(struct dma_buf_info *info, bool verify_buf_owner)
{
	...
	if (info->handle == NULL && info->tee_shm_file == NULL) {
		spcom_pr_err("shared buffer handle is NULL\n");
		return -EINVAL;
	}
	...
	if (info->tee_shm_file)
		fput(info->tee_shm_file);
	info->handle = NULL;
	info->tee_shm_file = NULL;
	info->fd = -1;
	info->owner_pid = 0;
...
static bool is_proc_channel_owner(struct spcom_channel *ch, u32 pid)
{
	int i = 0;
...
static int spcom_ioctl_handle_lock_dmabuf_command(struct spcom_ioctl_dmabuf_lock *arg)
{
	struct spcom_channel *ch = NULL;
	struct dma_buf *dma_buf = NULL;
	struct file *tee_shm_file = NULL;
	const char *ch_name = NULL;
	uint32_t pid = current_pid();
	int fd = 0;
	int i = 0;

	spcom_pr_dbg("Lock dmabuf cmd arg: ch_name[%s], fd[%d], padding[%u], PID[%u]\n",
	...
	if (IS_ERR_OR_NULL(dma_buf)) {
		dma_buf = NULL;
		tee_shm_file = spcom_get_tee_shm_file(fd);
		if (IS_ERR(tee_shm_file)) {
			spcom_pr_err("fail to get DMA-BUF or QTEE shared-memory handle\n");
			return PTR_ERR(tee_shm_file);
		}
	}
	...
	/* Check if this shared buffer is already locked */
	for (i = 0 ; i < ARRAY_SIZE(ch->dmabuf_array); i++) {
		if ((dma_buf && ch->dmabuf_array[i].handle == dma_buf) ||
		    (tee_shm_file &&
		     ch->dmabuf_array[i].tee_shm_file == tee_shm_file)) {
			spcom_pr_dbg("fd [%d] shared buf is already locked\n", fd);
			mutex_unlock(&ch->lock);
			if (dma_buf)
				dma_buf_put(dma_buf);
	...
	for (i = 0 ; i < ARRAY_SIZE(ch->dmabuf_array); i++) {
		...
		if (curr_buf->handle == NULL && curr_buf->tee_shm_file == NULL) {
			curr_buf->handle = dma_buf;
			curr_buf->tee_shm_file = tee_shm_file;
			curr_buf->fd = fd;
...
static int spcom_ioctl_handle_unlock_dmabuf_command(struct spcom_ioctl_dmabuf_lock *arg)
{
	...
	spcom_pr_dbg("Unlock dmabuf cmd arg: ch_name[%s], fd[%d], padding[%u], PID[%u]\n",
		arg->ch_name, arg->fd, arg->padding, current_pid());
	...
	if (!unlock_all) {
		...
		if (IS_ERR_OR_NULL(dma_buf)) {
			...
			tee_shm_file = spcom_get_tee_shm_file(fd);
			if (IS_ERR(tee_shm_file)) {
				spcom_pr_err("Failed to get shared buffer handle, fd [%d]\n", fd);
				return PTR_ERR(tee_shm_file);
			}
		}
	}
...
static bool is_arg_size_expected(unsigned int cmd, uint32_t arg_size)
{
	...
	case SPCOM_IOCTL_DMABUF_UNLOCK:
		...
		break;
	default:
		spcom_pr_err("No userspace data for ioctl cmd[%u]\n", cmd);
		return false;
	}

	if (arg_size != expected_size) {
		spcom_pr_err("Invalid cmd size: cmd[%u], arg size[%u], expected[%u]\n",
...
static int spcom_rpdev_probe(struct rpmsg_device *rpdev)
{
	const char *name;
	struct spcom_channel *ch;
	void __iomem *stop_polling;

	/* Stop polling. */
	stop_polling = ioremap(SPCOM_POLLING_REGISTER_ADDRESS, sizeof(u32));
	if (stop_polling) {
		spcom_pr_dbg("before stop polling register : %x",
			     readl_relaxed(stop_polling));
		writel_relaxed(SPCOM_STOP_POLLING_VALUE, stop_polling);
		spcom_pr_dbg("after stop polling register : %x",
			     readl_relaxed(stop_polling));
		iounmap(stop_polling);
	} else {
		spcom_pr_warn("failed to map stop polling register\n");
	}
...
static int spcom_probe(struct platform_device *pdev)
{
	...
	np = pdev->dev.of_node;
	prop = np ? of_find_property(np, "qcom,rproc-handle", NULL) : NULL;
	...
	if (np) {
		...
		strscpy(dev->predefined_ch_name[0], "sp_kernel",
